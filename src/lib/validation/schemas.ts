...
export const blogCreateSchema = Joi.object({
  ...
  seo: Joi.object({
    ...
    keywords: Joi.array().items(Joi.string().min(2).max(30)).max(10).default([])
  }).default({})
});

export const blogUpdateSchema = blogCreateSchema.fork(['title', 'content', 'excerpt', 'category'], (schema) => 
  schema.optional()
);
...
export const projectUpdateSchema = projectCreateSchema.fork(['title', 'description', 'shortDescription', 'technologies', 'category', 'startDate'], (schema) => 
  schema.optional()
);
...
export const testimonialUpdateSchema = testimonialCreateSchema.fork(['name', 'email', 'content', 'rating'], (schema) => 
  schema.optional()
);
...
export const fileUploadSchema = Joi.object({
  ...
  mimetype: Joi.string()
    ...
    .required()
    .messages({
      'any.only': 'File type not supported. Please upload images (JPEG, PNG, WebP, GIF) or documents (PDF, DOC, DOCX)'
    }),
  size: Joi.number()
